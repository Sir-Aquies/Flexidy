...
export class ProductsService {
  products: Product[] = this.ProductArray(300);
  ...
  async SynchronousProductArray(amount: number, size: Size = Size.default): Promise<Product[]> {
    ...
    while (amount > 0) {
      ...
      let pic = await this.getPicsumSync(Math.floor(Math.random() * 1085), product.size ? product.size : Size.default);

      if (pic.status === 200) {
        product.image = pic.url;
        output.push(product);
      }

      let author = await this.getJSONSync("https://random-data-api.com/api/users/random_user");

      product.author = `${(author as any).first_name} ${(author as any).last_name}`;
  ...
  private getPicsumSync(id: number, size: Size): Promise<any> {
    ...
    return this.http.get(`https://picsum.photos/id/${id}/${width}/${height}`, { observe: 'response', responseType: 'text' }).toPromise();
  }
